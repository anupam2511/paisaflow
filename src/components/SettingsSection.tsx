...
export default function SettingsSection({ data, setFinanceData, userEmail }: SettingsSectionProps) {
  ...
  // Local Storage Data Recovery & Manual backup states
  const [localKeys, setLocalKeys] = useState<{key: string; size: number; isCompatible: boolean; data?: any}[]>([]);
  ...
  const scanLocalStorageKeys = () => {
    const keys: {key: string; size: number; isCompatible: boolean; data?: any}[] = [];
      ...
      if (key) {
        ...
        if (val.trim().startsWith('{') || val.trim().startsWith('[')) {
          try {
            ...
            if (parsed && (parsed.expenses || parsed.investments || parsed.recurringSpends || parsed.ccTransactions || parsed.preferences)) {
              isCompatible = true;
              dataParsed = parsed;
            }
          } catch (e) {}
        }
        keys.push({ key, size: val.length, isCompatible, data: dataParsed });
      }
    }
    setLocalKeys(keys.sort((a, b) => b.size - a.size));
  };

  // Run scan once on load
  React.useEffect(() => {
  ...
  const handleImportJson = (jsonStr: string) => {
    try {
      if (!jsonStr.trim()) {
        setPastedError('Please paste some JSON first.');
        return;
      }
      const parsed = JSON.parse(jsonStr);
      if (!parsed || typeof parsed !== 'object') {
        setPastedError('Invalid JSON structure. Must be a JSON object.');
        return;
      }
      // Basic structures check
      if (!parsed.expenses && !parsed.investments && !parsed.ccTransactions) {
        if (!window.confirm('This JSON does not seem to contain standard financial records (expenses, investments, etc.). Do you want to import it anyway?')) {
          return;
        }
      }
      setFinanceData(parsed);
      setPastedError('');
      alert('Backup loaded successfully into current session! Your view has been updated.');
    } catch (err: any) {
      setPastedError(`Error parsing JSON: ${err.message || String(err)}`);
    }
  };

  const handleRestoreKey = (key: string, dataObj: any) => {
    if (!dataObj) return;
    if (window.confirm(`Are you sure you want to load the backup from key "${key}"? This will overwrite the current session metrics.`)) {
      setFinanceData(dataObj);
      alert(`Data successfully restored from browser key "${key}"!`);
    }
  };
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <h4 className="text-xs font-extrabold text-amber-700 dark:text-amber-400 flex items-center gap-1.5 mb-1">
                        <ShieldAlert className="w-4 h-4 text-amber-600 dark:text-amber-400" /> Local-First Offline Mode
                      </h4>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-normal font-medium">
                        Bypasses all remote database calls and operates exclusively in browser local storage. Enable this if you have network issues or Firestore connection errors in your sandbox.
                      </p>
                    </div>
                    <div className="relative inline-flex items-center cursor-pointer mt-0.5 shrink-0">
                      <input
                        ...
                        onChange={(e) => handleToggleForceOffline(e.target.checked)}
                ...
                <div className="flex border-b border-slate-100 dark:border-slate-800 gap-2 p-1 bg-slate-50 dark:bg-slate-950/60 rounded-xl">
                  <button
                    ...
                    className={`flex-1 text-[11px] font-bold py-2 rounded-lg transition-all ${activeTabSub === 'scan' ? 'bg-white dark:bg-[#0f1a36] text-indigo-600 dark:text-indigo-400 shadow-xs' : 'text-slate-500 dark:text-slate-400'}`}
                  >
                    Scan Keys
                  </button>
                  <button
                    type="button"
                    onClick={() => setActiveTabSub('import')}
                    className={`flex-1 text-[11px] font-bold py-2 rounded-lg transition-all ${activeTabSub === 'import' ? 'bg-white dark:bg-[#0f1a36] text-indigo-600 dark:text-indigo-400 shadow-xs' : 'text-slate-500 dark:text-slate-400'}`}
                  >
                    Import JSON
                  </button>
                  <button
                    ...
                    className={`flex-1 text-[11px] font-bold py-2 rounded-lg transition-all ${activeTabSub === 'export' ? 'bg-white dark:bg-[#0f1a36] text-indigo-600 dark:text-indigo-400 shadow-xs' : 'text-slate-500 dark:text-slate-400'}`}
                  >
                    Export
                  </button>
                </div>
                ...
                {activeTabSub === 'scan' && (
                  <div className="space-y-2 max-h-[170px] overflow-y-auto pr-1">
                    ...
                    {localKeys.length === 0 ? (
                      <p className="text-[11px] text-slate-400 text-center py-4 font-semibold">No browser cache keys found.</p>
                    ) : (
                      localKeys.map(({ key, size, isCompatible, data: keyData }) => (
                        <div key={key} className="p-3 bg-slate-50 dark:bg-slate-950/40 rounded-xl border border-slate-100 dark:border-slate-800/50 flex flex-col gap-2">
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              <p className="text-xs font-bold text-slate-700 dark:text-slate-300 truncate" title={key}>{key}</p>
                            ...
                            {isCompatible ? (
                              ...
                              <span className="text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-md font-bold shrink-0">
                                Generic Raw
                              </span>
                            )}
                          </div>
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (previewKey === key) {
                                  ...
                                  try {
                                    const val = localStorage.getItem(key) || '';
                                    setPreviewData(JSON.parse(val));
                                  } catch (e) {
                                    setPreviewData(localStorage.getItem(key));
                                  }
                                }
                              }}
                              className="flex-1 text-[10px] font-bold bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg py-1.5 transition text-center cursor-pointer"
                            >
                              {previewKey === key ? 'Close' : 'Preview'}
                            </button>
                          </div>

                          {previewKey === key && previewData && (
                            <div className="p-2.5 bg-slate-100 dark:bg-slate-900 rounded-lg text-[10px] font-mono text-slate-600 dark:text-slate-400 overflow-x-auto max-h-[120px] border border-slate-200 dark:border-slate-800">
                              {typeof previewData === 'object' ? (
                                <div>
                                  <div className="font-extrabold text-indigo-600 dark:text-indigo-400 mb-1">Found Objects:</div>
                                  {previewData.expenses && <div>• Expenses: {Array.isArray(previewData.expenses) ? previewData.expenses.length : 0}</div>}
                                  {previewData.investments && <div>• Investments: {Array.isArray(previewData.investments) ? previewData.investments.length : 0}</div>}
                                  {previewData.ccTransactions && <div>• CC Transactions: {Array.isArray(previewData.ccTransactions) ? previewData.ccTransactions.length : 0}</div>}
                                  <pre className="mt-2 text-[9px] text-slate-500 max-h-[60px] overflow-y-auto">
                                    {JSON.stringify(previewData, null, 2).slice(0, 500)}...
                                  </pre>
                                </div>
                              ) : (
                                <div className="truncate">{String(previewData)}</div>
                              )}
                            </div>
                          )}
                        </div>
                ...
                {activeTabSub === 'import' && (
                  <div className="space-y-3">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Import JSON Database Backup</span>
                    <textarea
                      value={pastedJson}
                      onChange={(e) => { setPastedJson(e.target.value); setPastedError(''); }}
                      placeholder='Paste backup {"expenses": [...], "investments": [...]} JSON data...'
                      className="w-full h-24 text-xs font-mono bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-300 p-3 rounded-xl border border-slate-200 dark:border-slate-800 focus:outline-none focus:border-indigo-500"
                    />
                    {pastedError && (
                      <p className="text-[10px] text-rose-500 font-extrabold flex items-center gap-1">
                        <AlertCircle className="w-3.5 h-3.5" /> {pastedError}
                      </p>
                    )}
                    <button
                      type="button"
                      onClick={() => handleImportJson(pastedJson)}
                      className="w-full text-xs text-white font-extrabold bg-indigo-600 hover:bg-indigo-700 py-2.5 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 shadow-xs"
                    >
                ...
                {activeTabSub === 'export' && (
                  <div className="space-y-3">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Raw JSON Export</span>
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={handleCopyCurrentData}
                        className="flex-1 text-xs text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50/50 dark:bg-indigo-950/30 hover:bg-indigo-100/50 border border-indigo-100 dark:border-indigo-900/40 py-2 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5"
                      >
                      ...
                      <a
                        ...
                        download="paisaflow_database_backup.json"
                        className="flex-1 text-xs text-white font-bold bg-indigo-600 hover:bg-indigo-700 py-2 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 shadow-xs text-center"
                      >
